...
public class OtherPreferencesActivity extends BasePreferencesActivity {
    ...
    private int accountNotificationsRow;
    ...
    protected void updateRowsId() {
        ...
        notificationsHeaderRow = newRow();
        accountNotificationsRow = newRow();
        notificationsDividerRow = newRow();
    ...
    protected void onItemClick(View view, int position, float x, float y) {
        if (position == accountNotificationsRow) {
            boolean current = AccountNotificationManager.getInstance().isCurrentAccountNotificationsEnabled();
    ...
    private class ListAdapter extends BaseListAdapter {
            ...
            switch (holder.getItemViewType()) {
                ...
                case 3:
                    ...
                    if (position == notificationsHeaderRow) {
                        headerCell.setText("Уведомления аккаунта");
                ...
                case 5:
                    ...
                    if (position == accountNotificationsRow) {
                        boolean enabled = AccountNotificationManager.getInstance().isCurrentAccountNotificationsEnabled();
                        checkCell.setTextAndValueAndCheck(
                                "Уведомления этого аккаунта",
                                "Включить или отключить все уведомления для текущего аккаунта",
                                enabled, true, true);
                    }
                    break;
                case 8:
        ...
        public int getItemViewType(int position) {
            if (position == deleteAccountDividerRow || position == notificationsDividerRow) {
                return 1;
            } else if (position == notificationsHeaderRow || position == analyticsHeaderRow) {
                return 3;
            } else if (position == accountNotificationsRow) {
                return 5;
